...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    if (!gpFile)
    {
        ...
        fprintf(gpFile, "Arjun Loaded Successfully\n\n");
    ...
    if (result != 0)
    {
        fprintf(gpFile, "initialize() failed. Exiting.\n");
        DestroyWindow(hwnd);
        hwnd = NULL;
    }
    else
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE:
        ZeroMemory(&wpPrev, sizeof(WINDOWPLACEMENT));
...
void display(void)
{
    ...
    drawNakul(xOrigin, yOrigin, fScaleFactor, 1);
...
void drawHandAndCloth(float xOrigin, float yOrigin, float scale, int iStanding)
{
...

    {
        ...
        drawTriangle(
            xOrigin + (-0.5950f * scale), yOrigin + (-0.010f * scale),
            xOrigin + (-0.6175f * scale), yOrigin + (0.020f * scale),
            xOrigin + (-0.6050f * scale), yOrigin + (0.024f * scale)
        ); //finger tips
        ...
        if (iStanding == 1)
        {
            drawQuadrangle(
                ...
                xOrigin + (-0.6100f * scale), yOrigin + (-0.010f * scale)
            ); //thumb
        }
        else
        {
            drawQuadrangle(
                ...
                xOrigin + (-0.6200f * scale), yOrigin + (-0.036f * scale)
            ); //thumb
        }
